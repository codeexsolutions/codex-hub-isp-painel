...
export default function AdminLoginPage({ onLogin, mensagem }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await Admin.login(usuario, senha);
      onLogin();
    } catch (err) {
      setErro(err.message);
    } finally {
    ...
    <div className="min-h-dvh flex items-center justify-center bg-canvas p-4">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="bg-surface rounded-3xl border border-border p-6 space-y-4 shadow-soft">
          <div>
            <h1 className="text-base text-text font-display">Acesso interno</h1>
            <p className="text-xs text-text-dim mt-1">Ativação de módulos por provedor.</p>
          </div>
